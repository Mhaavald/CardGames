...
namespace GameStudioB
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("Welcome to Card Games Studio!");
            Console.WriteLine("-------------------------");
            DisplayMainMenu();
        }
        ...
        private static void DisplayMainMenu()
        {
            ...
            while (!exitProgram)
            {
                ...
                Console.WriteLine("2. Play AceReset");
                Console.WriteLine("3. Run Strategy Simulation");
            ...
            Console.WriteLine("\nThank you for playing Card Games Studio!");
            Console.WriteLine("Press any key to exit...");
            Console.ReadKey();
        }

        private static void PlayBlackJackGame()
        {
        ...
        private static void PlayAceResetGame()
        {
            AceResetGame game = new AceResetGame(true);
            bool continuePlaying = true;
        ...
        private static void GameSelectionMenu()
        {
            ...
            while (!returnToMainMenu)
            {
                ...
                {
                    case "1":
                        BlackJackSimulationMenu();
                        break;
                    case "2":
                        AceResetSimulationMenu();
                        break;
                    case "3":
        ...
        private static void BlackJackSimulationMenu()
        {
            ...
            while (runSimulation)
            {
                ...
                Console.WriteLine("7. Return to Game Selection");
                Console.Write("\nSelect an option: ");
                ...
                if (rounds > 0 || (isTimeBased && minutes > 0))
                {
                    ...
                    if (isTimeBased)
                    {
                        RunBlackJackTimeBasedSimulation(strategies, minutes, verbose);
                    }
                    else
                    {
                        RunBlackJackSimulation(strategies, rounds, verbose);
                    }
                }
            }
        }
        
        private static void AceResetSimulationMenu()
        {
            Console.WriteLine("\nAceReset Strategy Simulation");
            Console.WriteLine("---------------------------");

            // Create the strategies
            IAceResetStrategy[] strategies = new IAceResetStrategy[]
            {
                new ConservativeAceResetStrategy(),
                new AggressiveAceResetStrategy(),
                new BalancedAceResetStrategy(),
                new ProbabilisticAceResetStrategy(),
                new RandomAceResetStrategy()
            };
            ...
            while (runSimulation)
            {
                ...
                if (rounds > 0 || (isTimeBased && minutes > 0))
                {
                    // Clear the console before starting a new simulation for better readability
                    Console.Clear();
                    Console.WriteLine("== AceReset Strategy Simulation ==");
                    
                    if (isTimeBased)
                    {
                        RunAceResetTimeBasedSimulation(strategies, minutes, verbose);
                    }
                    else
                    {
                        RunAceResetSimulation(strategies, rounds, verbose);
                    }
                }
            }
        }
        ...
        private static void RunBlackJackSimulation(IStrategy[] strategies, int rounds, bool verbose)
        {
        ...
        private static void RunBlackJackTimeBasedSimulation(IStrategy[] strategies, int minutes, bool verbose)
        {
        ...
        private static void RunAceResetSimulation(IAceResetStrategy[] strategies, int rounds, bool verbose)
        {
            Console.WriteLine($"\nRunning AceReset simulation with {rounds:N0} rounds per strategy...");
            
            var startTime = DateTime.Now;
            Console.WriteLine($"Simulation started at: {startTime:HH:mm:ss}");
            Console.WriteLine("=======================================");
            
            var simulator = new AceResetSimulator(strategies, verbose);
            var result = simulator.RunSimulation(rounds);
        ...
        private static void RunAceResetTimeBasedSimulation(IAceResetStrategy[] strategies, int minutes, bool verbose)
        {
            Console.WriteLine($"\nRunning AceReset time-based simulation for {minutes} minute{(minutes != 1 ? "s" : "")}...");
